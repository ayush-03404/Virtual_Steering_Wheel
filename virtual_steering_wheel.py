...
import asyncio, json, math, os, platform, queue
...
# ════════════════════════════════════════════════════════════════════════════════
# HTTP TELEMETRY  (Funbit ETS2 Telemetry Server)
# ════════════════════════════════════════════════════════════════════════════════
# Polls http://192.168.56.1:25555/api/ets2/telemetry every 100 ms.
# Works with both the ETS2 demo and the full game.
# Start the server with start_ets2.bat before launching this app.

_TELEM_URL      = "http://192.168.56.1:25555/api/ets2/telemetry"
_TELEM_INTERVAL = 0.10   # seconds between polls


class HttpTelemetryReader:
    """
    Polls the Funbit ETS2 Telemetry Server HTTP API every 100 ms.
    Falls back to simulated data when the server is not reachable.
    Works with both the ETS2 demo and the full game.
    """
    def __init__(self):
        self._speed = 0.0
    ...
    def _fetch(self):
        """Return (speed_kmh, rpm, gear) or None on failure / game not connected."""
        try:
            req = urllib.request.Request(_TELEM_URL,
                                         headers={'Accept': 'application/json'})
            with urllib.request.urlopen(req, timeout=0.4) as resp:
                data = json.loads(resp.read().decode())
            if not data.get('game', {}).get('connected', False):
                return None
            truck = data.get('truck', {})
            speed_ms = float(truck.get('speed', 0.0))
            rpm      = float(truck.get('engineRpm', 800.0))
            gear     = int(truck.get('displayedGear', 0))
            return speed_ms * 3.6, rpm, gear   # m/s → km/h
        except Exception:
            return None
    ...
    def _loop(self):
        ...
        while True:
            ...
            if result:
                ...
                if not was_live:
                    print('[TELEM] Funbit server connected — live gauges active.')
                    was_live = True
            else:
                with self._lock:
                    self._live = False
                if was_live:
                    print('[TELEM] Funbit server lost — using simulated data.')
                    was_live = False
            time.sleep(_TELEM_INTERVAL)
...
_telemetry = HttpTelemetryReader()   # singleton shared by both modes
...
#tlive{font-size:10px;color:#666;margin-left:8px}
...
<div id="app">
  <div id="st">
    <div style="display:flex;align-items:center">
      ...
      <span id="tlive" class="off">● SIM (demo)</span>
    </div>
    <div id="profs">
...
function tapShift(d){
  ...
  // Send immediately — don't wait for the 33 ms interval
...
function connect(){
  ...
  ws.onmessage=e=>{
    try{const d=JSON.parse(e.data);if(d.t){tSpeed=d.t.speed||0;tRpm=d.t.rpm||800;tGear=d.t.gear||0;tLive=!!d.t.live;tliveEl.textContent=tLive?'● LIVE':'● SIM (demo)';tliveEl.className=tLive?'on':'off';}}catch(err){}
  };
}
function sendCtrl(){
